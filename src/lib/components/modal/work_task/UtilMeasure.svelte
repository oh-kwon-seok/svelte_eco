...
<script>
    ...
    import {save,modalClose,measureModalTable,measureSave } from '$lib/store/work_task/function';
    ...
    afterUpdate(() => {
      ...
      if($work_task_form_state['modal'] === false){
        ...
        measureModalTable(table_modal_state,"measure",tableComponent);
        }
    });
    onMount(()=>{
    ...
    <Modal  class="w-full" title={`계량 등록`} permanent={true} color={color}  bind:open={$work_task_modal_state[title]['use']} size="xl"  placement={'center'}   >
        ...
        <div class="grid grid-cols-3 gap-4 ">
          ...
          <div class="grid grid-cols-1 gap-4">
            <p class="mb-4 font-semibold text-xl dark:text-white">계량 리스트</p>
   ...
   <svelte:fragment slot='footer'> 
    <Button  color={color}  class="w-1/2" on:click={measureSave($work_task_form_state,title)}>계량완료</Button>
