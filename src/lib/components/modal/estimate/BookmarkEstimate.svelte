...
<script>
    ...
    import {bookmarkEstimateSearchTable,bookmarkEstimateSearchModalClose} from '$lib/store/estimate/function';
    ...
    let tableComponent = "example-table-theme";
      ...
      onMount(()=>{
        bookmarkEstimateSearchTable(table_modal_state,"bookmark_estimate",tableComponent,"info_select",title);
      });
    ...
    <Modal title={`자동견적 선택`}  permanent={true} color={color} bind:open={$bookmark_estimate_modal_state[title]['use']} size="xl" placement={ 'center'}   class="w-full">
        ...
        <Button  class="w-full" color='red' on:click={bookmarkEstimateSearchModalClose(title)}>닫기</Button>
