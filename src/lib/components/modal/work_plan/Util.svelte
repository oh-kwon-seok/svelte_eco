...
<script>
    ...
    import {work_plan_modal_state, work_plan_form_state} from '$lib/store/work_plan/state';

    import BomSearch from '$lib/components/modal/work_plan/Bom.svelte';
    ...
    import {save,modalClose,bomSearchModalOpen } from '$lib/store/work_plan/function';
    ...
    <Modal  class="w-full" title={`생산계획 ${label_title}`} permanent={true} color={color}  bind:open={$work_plan_modal_state[title]['use']} size="xl"  placement={'center'}   >
        ...
        <div class="grid grid-cols-2 gap-4 ">
          ...
          <Label class="space-y-2">
            <span>품목코드</span>
            <Input type="text" id="last_name" placeholder="품목코드를 입력하세요" required bind:value={$work_plan_form_state['bom_code']} on:click={() => {bomSearchModalOpen('work_plan_bom_search')}}/>
            
            {#if $work_plan_form_state['bom_code'] === '' && $common_alert_state['value'] === true}
            <Helper class="mt-2" color="red"><span class="font-medium">데이터를 입력해주세요</span></Helper>
            {/if}
          </Label>

          {#if $bom_modal_state['title'] === 'work_plan_bom_search' }
          <BomSearch title="work_plan_bom_search" />
          {/if}
          ...
          <Label class="space-y-2">
            ...
            <Input type="date"   id="last_name" placeholder="생산 예정 시작 일자룰 입력하세요" required bind:value={$work_plan_form_state['start_date']}/>
          </Label>
          <Label class="space-y-2">
            <span>생산예정종료일자</span>

            <Input type="date"   id="last_name" placeholder="생산 예정 종료 일자룰 입력하세요" required bind:value={$work_plan_form_state['end_date']}/>
          </Label>

          <Label class="space-y-2">
            <span>수량</span>
            <Input type="text" id="last_name" placeholder="수량을 입력하세요" required bind:value={$work_plan_form_state['qty']} />
            
            {#if (parseFloat($work_plan_form_state['qty']) === 0 || parseFloat($work_plan_form_state['qty']) < 0)   && $common_alert_state['value'] === true}
          ...
          <Label class="space-y-2">
            <span>단위</span>
            <Input type="text" id="last_name" placeholder="단위를 입력하세요" required bind:value={$work_plan_form_state['unit']} />
            
            {#if parseFloat($work_plan_form_state['unit']) === "" && $common_alert_state['value'] === true}
          ...
          {#if $work_plan_modal_state['title'] === 'update'}
            <Label class="space-y-2">
              <span>사용유무</span>
              <Select id="countries" class="mt-2" bind:value={$work_plan_form_state['used']} placeholder="">
   ...
   <svelte:fragment slot='footer'> 
    <Button  color={color}  class="w-1/2" on:click={save($work_plan_form_state,title)}>{label_title}</Button>
