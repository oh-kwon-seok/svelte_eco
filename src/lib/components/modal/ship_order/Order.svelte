...
<script>
    ...
    import {orderSearchTable,orderSearchModalClose} from '$lib/store/ship_order/function';
    ...
    let tableComponent = "example-table-theme";
      ...
      onMount(()=>{
        orderSearchTable(table_modal_state,"order",tableComponent,"info_select",title);
      });
    ...
    <Modal title={`주문 선택`}  permanent={true} color={color} bind:open={$order_modal_state[title]['use']} size="xl" placement={ 'center'}   class="w-full">
        ...
        <Button  class="w-full" color='red' on:click={orderSearchModalClose(title)}>닫기</Button>
