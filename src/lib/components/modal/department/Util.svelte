...
<script>
    ...
    import {department_modal_state, department_form_state} from '$lib/store/department/state';
    import {common_alert_state, common_toast_state, common_department_state,common_company_filter_state} from '$lib/store/common/state';
    
    import {save,modalClose} from '$lib/store/department/function';
    ...
    <Modal title={`부서 ${label_title}`} permanent={true} color={color} bind:open={$department_modal_state[title]['use']} size="xl" placement={'center'}   class="w-full">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          {#if $department_modal_state['title'] === 'update'}
            <Label class="space-y-2">
              <span>사용유무</span>
              <Select id="countries" class="mt-2" bind:value={$department_form_state['used']} placeholder="">
       ...
       {/if}
        ...
        <svelte:fragment slot="footer">
          <Button  class="w-1/2"  color={title === 'add' || title === 'update'  ? 'blue' : 'red'}    on:click={save($department_form_state,title)}>{label_title}</Button>
