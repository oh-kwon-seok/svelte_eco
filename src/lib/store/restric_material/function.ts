...
import {restric_material_modal_state,restric_material_form_state,restric_material_status_state} from './state';
...
let restric_material_status_data : any;
...
restric_material_modal_state.subscribe((data) => {
    update_modal = data;
})

restric_material_form_state.subscribe((data) => {
    update_form = data;
})
restric_material_status_state.subscribe((data) => {
    restric_material_status_data = data;
})
...
const restricMaterialModalOpen = (data : any, title : any) => {
  ...
  console.log('title : ', title);
    ...
    restric_material_modal_state.update(() => update_modal);
    ...
    if(title === 'update' ){
          Object.keys(update_form).map((item)=> {    
            ...
            restric_material_form_state.update(() => update_form);
            restric_material_modal_state.update(() => update_modal);
           
    }
    if(title === 'check_delete'){
      let data =  table_list_data['restric_material'].getSelectedData();
 ...
 if(title === 'check_delete'){
      ...
      if(data.length === 0){
        ...
        if(uid_array.length > 0){
          ...
          const url = `${api}/restric_material/delete`
          try {
          ...
          axios.post(url,
            ...
            if(res.data !== undefined && res.data !== null && res.data !== '' ){
              ...
              restric_material_modal_state.update(() => update_modal);
              restric_material_form_state.update(()=> init_form_data);

              select_query('restric_material');
    ...
    if (!confirm("사용제한 원료를 업데이트하면 기존의 데이터가 전부 변경됩니다. 선택하시겠습니까? ")) {
      ...
      const config = {
        ...
        await axios.get(url,config).then(res=>{
          ...
          restric_material_status_data['title'] = "초기데이터 요청중...";
          restric_material_status_data['total_count'] = page_row;
          restric_material_status_data['ing_count'] = array.length;
          restric_material_status_data['final_total_count'] =0;
          restric_material_status_state.update(()=> restric_material_status_data);
       })
            ...
            const config = {
              ...
              await axios.get(url,config).then(res=>{
                ...
                if(array.length > 0){
                  ...
                  restric_material_status_data['title'] = "데이터 가공 및 누적중...";
                  restric_material_status_data['ing_count'] = array.length;
       ...
       let final_data = filteredData.map(obj => {
        ...
        restric_material_status_data['title'] = "데이터 입력중...";
        restric_material_status_data['ing_count'] = final_data.length;
        restric_material_status_data['final_total_count'] = final_data.length;
        ...
        const create_url = `${api}/restric_material/save`
        try {    
          console.log('storedUsername : ', storedUsername);   
          let params = {
            id : storedUsername,
            restric_material_array : final_data,
        ...
        axios.post(create_url,
          ...
          if(res.data !== undefined && res.data !== null && res.data !== '' ){
            ...
            restric_material_status_data['title'] = "";
            restric_material_status_data['ing_count'] = 0;
            restric_material_status_data['total_count'] = 0;
            restric_material_status_data['final_total_count'] =0;
...
export {restricMaterialModalOpen,save,modalClose,restrictUpdate}