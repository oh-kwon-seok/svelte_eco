...
<script>
    ...
    import Util from '$lib/components/modal/bookmark_estimate/Util.svelte';
    

    import * as Icon from 'svelte-awesome-icons';

    import {bookmarkEstimateModalOpen,bookmarkEstimateSubExcelFormDownload,bookmarkEstimateSubExcelUpload} from '$lib/store/bookmark_estimate/function';
    import {excelDownload, excelUpload, fileButtonClick,makeCustomTable} from '$lib/store/common/function';
    
    import {bookmark_estimate_form_state,bookmark_estimate_modal_state} from '$lib/store/bookmark_estimate/state';

    import {url_state,cookie_state,common_bookmark_estimate_state,table_list_state,common_toast_state,common_search_state} from '$lib/store/common/state';
    ...
    onMount(()=>{
        ...
        makeCustomTable(table_list_state,"bookmark_estimate",tableComponent,"select");
    ...
    afterUpdate(()=> {
        ...
        }else if($url_state['path'] === '/info/bookmark_estimate'){
    ...
    background-color: #444;
        ...
        <div class="grid grid-rows-16 grid-flow-col gap-1">
            ...
            <div class="col-span-1 row-span-1"> 
              <Title title='기준정보 관리' subtitle='자동견적레시피 관리'/>
            </div>
            ...
            <div class="row-span-15 col-span-12 "> 
                <Tabs  style="pill" defaultClass=" mt-5 overflow-auto  flex rounded-lg divide-x divide-gray-200 shadow dark:divide-gray-700" >
                    <TabItem  open >
                      ...
                      <SearchBar title="bookmark_estimate"/>


                      <div class='m-5'>

                        <Button  on:click={() => {bookmarkEstimateModalOpen('','add')}}>
                          <Icon.FloppyDiskSolid class='mr-2' size="20" />
                          추가
                        </Button>

                        <Button  color='red' on:click={() => bookmarkEstimateModalOpen('','check_delete')}>
                          <Icon.BanSolid class='mr-2' size="20" />
                          선택삭제
                        </Button>

                        <Button  color='green' on:click={() =>excelDownload('bookmark_estimate',EXCEL_CONFIG['bookmark_estimate'])}>
                    ...
                    <Button  color='green' on:click={(e)=> fileButtonClick('upload')}>
                      ...
                      on:change={(e)=> bookmarkEstimateSubExcelUpload(e)}
                
                      />
                  </Button>

                 
                    <Button  color="light" on:click={() => bookmarkEstimateSubExcelFormDownload()}>
                      ...
                      자동견적 항목 업로드 양식 다운
                        ...
                        {#if $bookmark_estimate_modal_state['title'] === 'add'}
                          <Util title="add" />
                        {:else if $bookmark_estimate_modal_state['title'] === 'update'}
                          <Util  title="update"/>
                          {:else if $bookmark_estimate_modal_state['title'] === 'check_delete'}
