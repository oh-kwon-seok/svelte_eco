...
<script>
    ...
    import Util from '$lib/components/modal/work_plan/Util.svelte';
    

    import * as Icon from 'svelte-awesome-icons';

    import {workPlanModalOpen,makeCustomTable} from '$lib/store/work_plan/function';
    import {excelDownload} from '$lib/store/common/function';
    
    import {work_plan_form_state,work_plan_modal_state} from '$lib/store/work_plan/state';

    import {url_state,cookie_state,production_work_plan_state,table_list_state,common_toast_state,common_search_state} from '$lib/store/common/state';
    ...
    onMount(()=>{
        ...
        makeCustomTable(table_list_state,"work_plan",tableComponent,"select");
    ...
    afterUpdate(()=> {
        ...
        }else if($url_state['path'] === '/production/work_plan'){
            ...
            <div class="row-span-15 col-span-12 "> 
                <Tabs  style="pill" defaultClass=" mt-5 overflow-auto  flex rounded-lg divide-x divide-gray-200 shadow dark:divide-gray-700" >
                    <TabItem  open >
                      ...
                      <SearchBar title="work_plan"/>


                      <div class='m-5'>

                        <Button  on:click={() => {workPlanModalOpen('','add')}}>
                          <Icon.FloppyDiskSolid class='mr-2' size="20" />
                          추가
                        </Button>

                        <Button  color='red' on:click={() => workPlanModalOpen('','check_delete')}>
                        ...
                        {#if $work_plan_modal_state['title'] === 'add'}
                          <Util title="add" />
                        {:else if $work_plan_modal_state['title'] === 'update'}
                          <Util  title="update"/>
                          {:else if $work_plan_modal_state['title'] === 'check_delete'}
