<script lang="ts">
    ...
    import { makeCustomTable, select_query } from '$lib/store/monitoring_product_stats/function';
  ...
  <div class="grid grid-rows-16 grid-flow-col gap-1">
    ...
    <div class="col-span-1 row-span-1">
        <Title title='모니터링' subtitle='제조리드타임 현황' />
    </div>
    <div class="row-span-15 col-span-12">
        <Tabs style="pill" defaultClass="mt-5 overflow-auto flex rounded-lg divide-x divide-gray-200 shadow dark:divide-gray-700">
            <TabItem open>
                <span slot="title">제조리드타임 현황</span>
                <div class="flex gap-2">
                    ...
                    <div class="w-2/5">
                        ...
                        <Input class="border-2 rounded-md p-2" id="search" placeholder="내용을 입력하세요" bind:value={$common_search_state['search_text']}>
                            ...
                            <Button slot="right" size="sm" type="submit" on:click={() => select_query('work_task', temp_chart)}>검색</Button>
                        </Input>
                    </div>
                </div>
                <div class="m-5"></div>
